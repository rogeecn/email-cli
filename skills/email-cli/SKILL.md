...
Run `email-cli --help` and `<command> --help` to discover the installed command surface.
...
- Commands are provider-neutral: `email-cli <command> -A alias`; the selected account determines the backend.
- TOML accounts: `-A alias` (`provider = "mailclaw"` or an IMAP provider); without it, commands use `default_account`. `-c path` selects another TOML file.
...
email-cli list -A cloud --format json --limit 20 --offset 0
email-cli list -A cloud --q 'invoice' --from billing@example.com --after 2026-01-01 --format json
email-cli get -A cloud 'email-string-id' --format json
email-cli attachments -A cloud 'email-string-id' --format json
email-cli health -A cloud --format json
```
...
```bash
email-cli send -A cloud --from sender@example.com --to recipient@example.com --subject 'Subject' --text-file ./approved-body.txt
email-cli delete -A cloud 'approved-email-string-id' --yes
```
